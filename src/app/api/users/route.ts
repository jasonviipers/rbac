...
import { UserFormSchema } from "@/lib/validators/schema";
...
export async function GET(req: NextRequest) {
    try {
        ...
        console.log("=========>function GET", findUsers);
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { name, email, hashedPassword } = validationResult.data;
        ...
        const newUser: NewUser = {
            ...
            email,
            emailVerified: false,
            hashedPassword: await new Scrypt().hash(hashedPassword),
            createdAt: new Date(),
        };
